...
import { PathCommandType, PickableType } from "../level.interface";
import type {Pickable } from "../level.interface";
...
export class Renderer {
  ...
  activeLayer!: Layer; // Using ! because it's assigned during activation

  // Declare properties without initializing them here
  baseLayer!: Layer;
  skyLayer!: Layer;
  terrainLayer!: Layer;
  hillsLayers!: Layer[];
  ...
  constructor(public engine: Engine) {
    // 1. Initialize properties inside the constructor body
    this.baseLayer = new Layer(this.engine, {
      canvas_: this.engine.canvas_,
    });

    this.skyLayer = new Layer(this.engine, {});

    this.terrainLayer = new Layer(this.engine, {
    ...
    // 2. Setup the main context
    const context = this.engine.canvas_.getContext("2d");
    if (!context) throw new Error("Main context failed");
    this.ctx = context;
  }

  init() {
    // Using a safe access to Layer.layers
  ...
  updateSize() {
    // Use window.innerWidth/Height for better reliability in Vite/React
